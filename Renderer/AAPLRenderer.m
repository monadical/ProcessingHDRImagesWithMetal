...
// The app's animation path is fixed, but these values will ensure that the number of steps in the path is reasonable
...
//   This MetalKit view object will also be used to set the pixelFormat and other properties of the drawable
- (nonnull instancetype)initWithMetalKitView:(nonnull MTKView *)mtkView cameraStepCount:(NSUInteger)cameraSteps resolutionScale:(float)resolutionScale
{
    ...
    if(self)
    {
        ...
        _cameraStepCount = (cameraSteps < kCameraAnimationMaxStepCount) ? ((cameraSteps > kCameraAnimationMinStepCount) ? cameraSteps : kCameraAnimationMinStepCount) : kCameraAnimationMaxStepCount;

        _resolutionScale = fClamp(kMinimumResolutionScale, kMaximumResolutionScale, resolutionScale);

        [self loadMetal:mtkView];
        [self onSizeUpdated:mtkView.bounds.size];

        // Don't let the drawable resize, we'll manage this manually.
#if defined(TARGET_IOS) || defined(TARGET_TVOS)
...
#else
    NSLog(@"Method call 'updateWithDevice:andView:' only applies to macOS.");
#endif
}
...
- (void) loadMetal:(nonnull MTKView *)mtkView
{
    ...
    NSError *error = nil;
    _skyDomeTexture = texture_from_radiance_file(@"table_mountain_1_4k.hdr", _device, &error);
    ...
    if(!_geometryPipeline)
    {
        NSLog(@"Error when creating geometry pipeline state: %@", error);
    }
    ...
    if(!_skyDomePipeline)
    {
        NSLog(@"Error when creating sky dome pipeline state: %@", error);
    }
    ...
    if (!_logLuminancePipeline)
    {
        NSLog(@"Error when creating log luminance pipeline state: %@", error);
    }
    ...
    for (uint32_t exposureControlTypeIdx = 0; exposureControlTypeIdx < kExposureControlTypeCount; ++exposureControlTypeIdx)
    {
        ...
        id<MTLFunction> fragFunc = [defaultLibrary newFunctionWithName:@"BloomSetup" constantValues:constantValues error:&variantError];
        if (!fragFunc)
        {
            NSLog(@"Error when creating BloomInit function variant: %@", variantError);
        }
        ...
        variantError = nil;
        _bloomInitPipelineVariants[exposureControlTypeIdx] = [_device newRenderPipelineStateWithDescriptor:pipelineDescriptor error:&variantError];

        if (!_bloomInitPipelineVariants[exposureControlTypeIdx])
        {
            NSLog(@"Error when creating BloomSetup pipeline variant: %@", variantError);
        }
    }
    ...
    for (uint32_t exposureControlTypeIdx = 0; exposureControlTypeIdx < kExposureControlTypeCount; ++exposureControlTypeIdx)
    {
        for (uint32_t tonemapOperatorTypeIdx = 0; tonemapOperatorTypeIdx < kTonemapOperatorTypeCount; ++tonemapOperatorTypeIdx)
        {
            ...
            NSError * variantError = nil;
            id<MTLFunction> fragFunc = [defaultLibrary newFunctionWithName:@"PostProcessComposite" constantValues:constantValues error:&variantError];
            ...
            variantError = nil;
            _compositePipelineVariants[exposureControlTypeIdx][tonemapOperatorTypeIdx] = [_device newRenderPipelineStateWithDescriptor:pipelineDescriptor error:&variantError];

            if (!_compositePipelineVariants[exposureControlTypeIdx][tonemapOperatorTypeIdx])
            {
                NSLog(@"Error when creating BloomInit pipeline variant: %@", variantError);
            }
        }
    }
    ...
    _sphereVertexBuffer = [_device newBufferWithBytes:sphereVerts length:_numSphereVerts * sizeof(AAPLVertex) options:MTLResourceOptionCPUCacheModeDefault];
    ...
    {
        _dynamicUniformBuffers[i] = [_device newBufferWithLength:sizeof(AAPLUniforms) options:MTLResourceStorageModeShared];
...
- (void)onSizeUpdated:(CGSize)size
{
    ...
    // Recompute the final render target sizes based upon resolution equivalent and aspect ration
    float resultWidth = size.width * _resolutionScale;
    float resultHeight = size.height * _resolutionScale;

    MTLTextureDescriptor * texDesc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:_sceneColorPixelFormat width:resultWidth height:resultHeight mipmapped:NO];
    texDesc.usage = MTLTextureUsageShaderRead | MTLTextureUsageRenderTarget;
    texDesc.storageMode = MTLStorageModePrivate;
    _sceneLinearColorTexture = [_device newTextureWithDescriptor:texDesc];

    texDesc = nil;
    texDesc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:_logLuminanceFormat width:size.width * kLogLuminanceTargetScale height:size.height * kLogLuminanceTargetScale mipmapped:YES];
    texDesc.usage = MTLTextureUsageShaderRead | MTLTextureUsageRenderTarget;
    texDesc.storageMode = MTLStorageModePrivate;
    _logLuminanceTexture = [_device newTextureWithDescriptor:texDesc];

    texDesc = nil;
    texDesc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:_sceneDepthPixelFormat width:resultWidth height:resultHeight mipmapped:NO];
    texDesc.usage = MTLTextureUsageRenderTarget;

#ifdef TARGET_MACOS
    texDesc.storageMode = MTLStorageModePrivate;
#else
    // Apple A-series platforms: Depth will not be reused in the scene, so it can be marked as memoryless.
    texDesc.storageMode = MTLStorageModeMemoryless;
#endif
    _sceneDepthTexture = [_device newTextureWithDescriptor:texDesc];

    // Create the collection of MTLTexture for the bloom chain
    // - Due to separable gaussian, we'll need two for each downsample level
    ...
    for (uint32_t bloomTargetScaleIdx = 0; bloomTargetScaleIdx < kBloomTargetScaleCount; ++bloomTargetScaleIdx)
    {
        for (uint32_t blurDirectionIdx = 0; blurDirectionIdx < kBlurDirectionCount; ++blurDirectionIdx)
        {
            ...
            texDesc = [MTLTextureDescriptor texture2DDescriptorWithPixelFormat:_bloomPixelFormat width:currWidth height:currHeight mipmapped:NO];
            texDesc.usage = MTLTextureUsageShaderRead | MTLTextureUsageRenderTarget;
            texDesc.storageMode = MTLStorageModePrivate;
...
- (void) updateState
{
    ...
    {
        uniforms->World[objIdx] = matrix4x4_translation(VEC3(sin(radians_from_degrees(offset)) * kObjectDistanceScale, 0, cos(radians_from_degrees(offset)) * kObjectDistanceScale));
        offset += kOffsetIncrement;
    }

    // Update the view and view inverse matrix
    ...
    uniforms->bloomParameters = VEC4(_bloomThreshold - _bloomRange, _bloomThreshold + _bloomRange, _bloomIntensity, 1.f);
...
{    
    ...
    id<MTLCommandBuffer> sceneCommandBuffer = [_commandQueue commandBuffer];
    sceneCommandBuffer.label = [NSString stringWithFormat:@"Scene CommandBuffer %lu", _cameraAnimationFrameIndex];
    ...
    [sceneCommandBuffer addCompletedHandler:^(id<MTLCommandBuffer> cb)
    {
    ...
    // Forward Pass
    {
        ...
        id<MTLRenderCommandEncoder> rce = [sceneCommandBuffer renderCommandEncoderWithDescriptor:rpd];
    ...
    // MARK: -- Scene Exposure I
    /*
     When logically laid out, scene exposure calculation happens prior to bloom setup; afterall,
       bloom setup takes exposure into account. However, for efficiency, exposure calculation is
       triggered after the frame presents to ensure ideal overlap and hardware utilization. Thus
       the exposure calculation won't be sent to the GPU until after the work for this command
       buffer is committed.
     */
    ...
    for (uint32_t bloomPassIdx = 0; bloomPassIdx < kBloomPassCount; ++bloomPassIdx)
    {
        ...
        id<MTLRenderCommandEncoder> rce = [sceneCommandBuffer renderCommandEncoderWithDescriptor:rpd];
        rce.label = [NSString stringWithFormat:@"Bloom Blur: %@ - From %d to %d", (direction == kBlurDirectionX) ? @"X" : @"Y", srcBloomTextureIdx, dstBloomTextureIdx];
    ...
    // MARK: -- Bloom Composite & Tone Mapping
    // Takes the 'final' version of the scene and maps into the desired range.
    MTLRenderPassDescriptor * passDesc = view.currentRenderPassDescriptor;
    if (passDesc)
    {
        passDesc.colorAttachments[0].clearColor = MTLClearColorMake(0.23, 0.23, 0.23, 1.0);

        id<MTLRenderCommandEncoder> rce = [sceneCommandBuffer renderCommandEncoderWithDescriptor:passDesc];
        rce.label = [NSString stringWithFormat:@"Bloom Composite + Tonemapping(%@)", string_for_tonemap_operator_type(_tonemapType)];
        ...
        [rce setFragmentTexture:_sceneLinearColorTexture atIndex:0];
        [rce setFragmentTexture:_bloomTargets[dstBloomTextureIdx] atIndex:1];
    ...
    [sceneCommandBuffer presentDrawable:view.currentDrawable afterMinimumDuration:1.f / kDesiredFrameRate];

    [sceneCommandBuffer commit];

    // MARK: -- Scene Exposure II
    // As mentioned in the section above, while exposure is calculated one frame behind,
    //   it can be logically considered as a step prior to bloom.
    if (_exposureType == kExposureControlTypeKey)
    {
