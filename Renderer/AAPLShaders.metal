...
/*
 ...
 Let's say that we have an input color, represented by a 3D vector, S and and luminance vector R,
 we may then compute input color luminance L by calculating the dot product of S and R:

   L = S・R

 Using L, we can then calculate the desired luminance L' using the tonemapping operator T(x) such that:

   L' = T(L)

 We now would like to determine the scalar value K such that:

  KS・R = L'

 By leveraging the scalar multiplication property of dot products, we can rewrite this as:
 ...
 And now we know that for any tone mapping operator T(x) which operates on Luminance, the color scaling factor K is:

   K = T(L) / L
*/

// Equation 3
half3 RinehardOperator(half3 srcColor)
{
    float luminance = dot(srcColor, ::kRec709Luma) + kLuminanceEpsilon;
    float scale = 1.f / (1.f + luminance);
    return srcColor * scale;
}

// Equation 4
half3 RinehardExOperator(half3 srcColor, half luminanceWhitePoint)
{
    float luminance = dot(srcColor, ::kRec709Luma) + kLuminanceEpsilon;

    float targetLum = (luminance * (1.f + (luminance / (luminanceWhitePoint * luminanceWhitePoint)))) / (1.f + luminance);

    return srcColor * (targetLum / luminance);
}
...
// Vertex shader outputs and per-fragment inputs.  Includes clip-space
// position and vertex outputs interpolated by rasterizer and fed
// to each fragment generated by clip-space primitives.
struct FSQVertexOut
{
    float4 position [[position]];
    float2 texCoord;
...
// Similar to FSQVertex function, but additionally a constant which represents how much to scale texel offsets based upon relative source target size
                                  ...
                                  const device float &srcTexelScale [[buffer(AAPLBufferIndexBytes)]])
{
    ...
    // Each bloom pass will send a texel scale value to calculate the correct offset into the source texture
    // This makes it easier for the app to control bloom pass count and quality
                                     ...
                                     const device AAPLUniforms& uniforms [[buffer(AAPLBufferIndexUniforms)]])
{
    ...
    switch (::kTonemapModeIndex)
    {
        ...
        {
            finalColor = ::RinehardOperator(finalColor);
            break;
        }

        case kTonemapOperatorTypeReinhardEx:
        {
            finalColor = ::RinehardExOperator(finalColor, uniforms.tonemapWhitePoint);
            break;
        }
